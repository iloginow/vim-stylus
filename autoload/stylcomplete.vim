...
function! stylcomplete#CompleteStyl(findstart, base)
  ...
  let curr_el = synIDattr(synID(line("."), col("."), 1), "name")

  if borders[max(keys(borders))] == 'firstword'
  ...
  elseif borders[max(keys(borders))] == 'secondword'
    ...
    elseif prop == 'transition-property'
      let values = ["all", "none"] + g:css_props
    ...
    elseif prop == 'transition'
      let values = ["all", "none"] + g:css_props + timing_functions
    ...
    elseif prop == 'will-change'
      let values = ["auto", "scroll-position", "contents"] + g:css_props
