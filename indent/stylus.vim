...
function! GetStylusIndent()
  ...
  if group =~? '\v^%(
        \stylusSelectorClass|
        \stylusSelectorId|
        \stylusSelectorElement|
        \stylusSelectorAttribute|
        \stylusSelectorPseudo|
        \stylusSelectorOperator|
        \cssSelectorEnclosures|
        \)$'
    return increase
  elseif (group == 'stylusFunction') && (indent(lnum) == '0') " mixin definition
    return increase
  else
    return indent
  endif
